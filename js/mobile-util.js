 ...
 *          对应 css 开发，任何弹性尺寸均使用 rem 单位，rem 默认宽度为 视觉稿宽度 / 16;   100px
 *              scss 中 $ppr(pixel per rem) 变量写法 -- $ppr: 750px/16/1rem; 640    40
		...
		fixScreen: function() {
            ...
            if ( !metaEl ) { // REM
                var docEl = doc.documentElement,//html
                    maxwidth = docEl.dataset.mw || 750, // 每 dpr 最大页面宽度 控制最大宽度
                    dpr = isIos ? Math.min(win.devicePixelRatio, 3) : 1,
                    scale = 1 / dpr, //比例
                    tid;

                docEl.removeAttribute('data-mw');
                docEl.dataset.dpr = dpr;//html 设置属性
                ...
                var refreshRem = function() {
                    var width = docEl.getBoundingClientRect().width;// 6 plus 1242 html的实际宽
                    if (width / dpr > maxwidth) {
                        width = maxwidth * dpr;
                    }
                    var rem = width / 16;  //比例  body width?? 100%     100rem   16rem   7.5rem
